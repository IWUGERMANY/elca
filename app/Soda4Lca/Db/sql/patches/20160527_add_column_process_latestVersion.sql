BEGIN;
SELECT public.register_patch('add_column_process::latestVersion', 'soda4lca');


ALTER TABLE soda4lca.processes ADD COLUMN "latest_version" varchar(50) DEFAULT NULL;
ALTER TABLE soda4lca.processes ALTER COLUMN "version" DROP NOT NULL;
...
CREATE VIEW soda4lca.processes_v AS
  SELECT DISTINCT p.import_id
    ...
    , CASE WHEN count(DISTINCT a.life_cycle_name) > 0 THEN array_to_string(array_accum(DISTINCT a.life_cycle_name), ', ')
      ELSE p.epd_modules
      END AS modules
...
CREATE VIEW soda4lca.processes_with_process_configs_v AS
  SELECT DISTINCT p.import_id
    ...
    , p.details
    , COALESCE(c.ref_num ||' '|| a.name_orig, p.class_id||' '||p.name) AS name
    , array_to_string(array_accum(DISTINCT '"'||pc.name||'"'), ', ') AS process_configs
    , CASE WHEN count(DISTINCT a.life_cycle_name) > 0 THEN array_to_string(array_accum(DISTINCT a.life_cycle_name), ', ')
      ELSE p.epd_modules
      END AS modules
  FROM soda4lca.processes         p
...
UPDATE soda4lca.processes SET latest_version=version, version='' WHERE status='SKIPPED' AND version IS NOT NULL;
