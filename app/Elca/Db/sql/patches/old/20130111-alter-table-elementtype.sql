BEGIN;
SELECT public.register_patch('alter-table-element-type', 'elca');
ALTER TABLE elca.element_types ADD COLUMN "pref_inclination"            int;
ALTER TABLE elca.element_types ADD COLUMN "pref_has_element_image"      boolean     NOT NULL    default false;

UPDATE elca.element_types
   SET pref_inclination = 0
     , pref_has_element_image = true
 WHERE is_constructional;
...
CREATE VIEW elca_cache.report_effects_v AS
    SELECT e.id AS element_id
      ...
      JOIN elca_cache.indicators ci ON ce.item_id = ci.item_id AND ci.life_cycle_ident = 'total'
