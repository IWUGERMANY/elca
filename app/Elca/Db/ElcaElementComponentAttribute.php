 ...
 *
 * @package    elca
 * @class      ElcaElementComponentAttribute
 * @author Fabian Möller <fab@beibob.de>
 * @author Tobias Lode <tobias@beibob.de>
 * @copyright  2014 BEIBOB Medienfreunde
 */
class ElcaElementComponentAttribute extends DbObject
{
    /**
     * Tablename
     */
    const TABLE_NAME = 'elca.element_component_attributes';

    /**
     * elementComponentAttributeId
     */
    private $id;
     ...
     * @return mixed
     */
    public static function findValue($elementComponentId, $ident, $force = false)
    {
        $Attribute = ElcaElementComponentAttribute::findByElementComponentIdAndIdent($elementComponentId, $ident, $force);
     ...
     */
    public static function updateValue($elementComponentId, $ident, $value, $forceTextValue = false)
    {
        ...
        $Attribute = ElcaElementComponentAttribute::findByElementComponentIdAndIdent($elementComponentId, $ident);
        ...
        if ($Attribute->isInitialized()) {
            ...
            $Attribute = ElcaElementComponentAttribute::create($elementComponentId, $ident, $numValue, $textValue);
        }
        return $Attribute;
    }
    // End updateValue
     ...
     * @return ElcaElementComponentAttribute
     */
    public static function create($elementComponentId, $ident, $numericValue = null, $textValue = null)
    {
        $ElcaElementComponentAttribute = new ElcaElementComponentAttribute();
        $ElcaElementComponentAttribute->setElementComponentId($elementComponentId);
        $ElcaElementComponentAttribute->setIdent($ident);
        $ElcaElementComponentAttribute->setNumericValue($numericValue);
        $ElcaElementComponentAttribute->setTextValue($textValue);
        
        if($ElcaElementComponentAttribute->getValidator()->isValid())
            $ElcaElementComponentAttribute->insert();
        return $ElcaElementComponentAttribute;
    }
    // End create
    ...
    /**
     * Inits a `ElcaElementComponentAttribute' by its primary key
     *
     * @param  int      $id    - elementComponentAttributeId
    ...
    {
        if(!$id)
            return new ElcaElementComponentAttribute();
    ...
    /**
     * Inits a `ElcaElementComponentAttribute' by its unique key (elementComponentId, ident)
     *
     * @param  int      $elementComponentId - elementComponentId
     * @param  string   $ident          - attribute identifier
     * @param  bool     $force          - Bypass caching
     * @return ElcaElementComponentAttribute
     */
    public static function findByElementComponentIdAndIdent($elementComponentId, $ident, $force = false)
    {
        if(!$elementComponentId || !$ident)
            return new ElcaElementComponentAttribute();
        ...
        return self::findBySql(get_class(), $sql, array('elementComponentId' => $elementComponentId, 'ident' => $ident), $force);
    }
    // End findByElementComponentIdAndIdent
     ...
     * @return bool
     */
    public static function existsByElementComponentIdAndIdent($elementComponentId, $ident, $force = false)
    {
        return self::findByElementComponentIdAndIdent($elementComponentId, $ident, $force)->isInitialized();
    }
    // End exists
    ...
    protected function insert()
    {
        ...
        $sql = sprintf("INSERT INTO %s (id, element_component_id, ident, numeric_value, text_value)
                               VALUES  (:id, :elementComponentId, :ident, :numericValue, :textValue)"
                       , self::TABLE_NAME
                       );
...
// End class ElcaElementComponentAttribute