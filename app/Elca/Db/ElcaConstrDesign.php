 ...
 *
 * @translate db Elca\Db\ElcaConstrDesignSet::find() name
 */
class ElcaConstrDesign extends DbObject
{
    /**
     * Tablename
     */
    const TABLE_NAME = 'elca.constr_designs';
     ...
     * Creates the object
     *
     * @param  string   $name - name of construction design
     * @param  string   $ident - internal short name
     */
    public static function create($name, $ident = null)
    {
        $ElcaConstrDesign = new ElcaConstrDesign();
        $ElcaConstrDesign->setName($name);
        $ElcaConstrDesign->setIdent($ident);

        if($ElcaConstrDesign->getValidator()->isValid())
            $ElcaConstrDesign->insert();
    ...
    /**
     * Inits a `ElcaConstrDesign' by its primary key
     *
     * @param  integer  $id    - constrDesignId
