...
use Elca\View\ElcaUsersMailView;
use Elca\View\ElcaUserView;
use Exception;

/**
 * Another user controller - messages to users
 *
 * @package elca
 ...
 *
 */
class UsersMailCtrl extends TabsCtrl
{
    /**
     * userId
     */
    ...
    protected function defaultAction($userId = null)
    {
        ...
        if(is_numeric($userId))
        {
            ...
            {
                $this->addTabItem('general', t('Allgemein'), null, 'Elca\Controller\UsersMailCtrl', 'general', ['u' => $User->getId()]);

                if($this->Access->hasAdminPrivileges()) {
                    $this->addTabItem('role', t('Rollen'), null, 'Elca\Controller\UsersMailCtrl', 'role', ['u' => $User->getId()]);
                }

                $this->invokeTabActionController();

                if ($this->Access->hasAdminPrivileges()) {
                    $this->Osit->add(new ElcaOsitItem(t('Benutzer'), '/users-mail/', t('Nutzerbenachrichtigung')));
                }
            ...
            else
            {
                if ($this->Access->isOrganisation() && $this->userId == $this->Access->getUserId())
                    ...
                    $this->noAccessRedirect('/users-mail/' . $this->Access->getUserId() . '/');
            }
        }
        else
        {
    ...
    /**
     * deactivate action - if login_time = null or login_time >= date-x
     */
    protected function deactivateAction()
    {
		...
		if(!$this->Request->status || $this->Request->status!="nologin")
			return;
			...
			foreach($Users as $user)
			{
				...
				try
				{
					...
					if($user->isInitialized())
					{
						...
						// $user->setDeactivatedmail(self::DEACTIVATEDMAILSEND);
						$user->updateDeactivate();

						$Dbh->commit();

						// Refresh cached object ??? Muss man das hier auch durchführen?
...
//            $Validator->assertTrue('gender', in_array($this->Request->gender, [UserProfile::GENDER_FEMALE, UserProfile::GENDER_MALE]), t('Bitte wählen Sie eine Anrede'));
        ...
        if($Validator->isValid())
        {
            ...
            try
            {
                ...
                if($user->isInitialized())
                {
                    ...
                    $this->updateHashUrl('/users-mail/'. $user->getId() .'/', false, true);
                    ...
                    if ($reloadView)
                    {
                         ...
                         * Render view by default action
                         */
                        $this->getView()->assign('redirect', '/users-mail/'. $user->getId() .'/');
                    }
                }
                else
                {
    ...
    {
        $this->addTabItem('general', t('Allgemein'), null, 'Elca\Controller\UsersMailCtrl', 'general');
