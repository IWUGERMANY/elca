...
class ProjectImportView extends HtmlView
{
    ...
    const URL_PATH_DOCS_DOWNLOADS_EXAMPLE_IFC_FILE = '/docs/downloads/IFC2eLCA/IFC2eLCA_Vorlage_BBSR-04-2020.ifc';
    ...
    {
        $container = $this->appendChild($this->getDiv(['id' => 'content', 'class' => 'project project-ifc-import']));

        $form = new HtmlForm('ifcImportForm', '/project-ifc/validate/');
        $form->setAttribute('id', 'ifcImportForm');
    ...
    protected function appendImportGroup(HtmlForm $form): void
    {
        ...
        $group->add(
            new HtmlTag(
                'a',
                t('Download IFC-Vorlage'),
                [
                    'href' => self::URL_PATH_DOCS_DOWNLOADS_EXAMPLE_IFC_FILE,
                    'target' => 'blank',
        ...
        $group->add(new ElcaHtmlFormElementLabel(t('Datei (.ifc)'), new HtmlUploadInput('importFile',null,false,null,null,'.ifc')));
