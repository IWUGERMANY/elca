...
/**
 * Builds a user sheet with extended information
 *
 * @package elca 
 ...
 *
 */
class ElcaUserSheetAdvancedView extends ElcaSheetView
{
    /**
     * User
     */
    ...
    protected function beforeRender()
    {
        ...
        if ($this->get('canEdit', false))
            $this->addFunction('edit', '/users-mail/$$itemId$$/', t('Bearbeiten'), 'default page');

        if ($this->User->getId() != $this->get('currentUserId')) {
            if ($this->get('hasAdminPrivileges', false))
                $this->addFunction('delete', '/users-mail/delete/?id=$$itemId$$', t('Benutzer löschen, ohne die Projektdaten zu löschen'));

            if ($this->get('hasAdminPrivileges', false))
                $this->addFunction('delete-recursive', '/users-mail/delete/?id=$$itemId$$&recursive', t('Benutzer mitsamt seinen Projektdaten löschen'));
        }
         ...
         */
		 ...
		 /**
         * Last Login not valid - show created / modified dates
         */
        $LastLogin = BlibsDateTime::factory($this->User->getLoginTime());
		$CreatedDate = BlibsDateTime::factory($this->User->getCreated());
		$ModifiedDate = BlibsDateTime::factory($this->User->getModified()); 
		...
		// line feed
		$this->addInfo(t(''), t(''),null,true);
		
		if(!$LastLogin->isValid())
		{
			$this->addInfo(t('nie'), t('Letzte Anmeldung'));
			$this->addInfo($CreatedDate->isValid() ? $CreatedDate->getDateTimeString(t('DATETIME_FORMAT_DMY') . ' ' . t('DATETIME_FORMAT_HI')) : t('nie'), t('Erstellt'));
			$this->addInfo($ModifiedDate->isValid() ? $ModifiedDate->getDateTimeString(t('DATETIME_FORMAT_DMY') . ' ' . t('DATETIME_FORMAT_HI')) : t('nie'), t('Geändert'));
		}	
		else 
		{
			$this->addInfo($LastLogin->getDateTimeString(t('DATETIME_FORMAT_DMY') . ' ' . t('DATETIME_FORMAT_HI')), t('Letzte Anmeldung'));
		}
