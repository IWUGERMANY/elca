<div id="content" class="imprint">
    <h1>Imprint</h1>
    <h2>Operator</h2>
    <p>Bundesinstitut für Bau-, Stadt-und Raumforschung (BBSR)<br/>
        im Bundesamt für Bauwesen und Raumordnung (BBR)<br/>
        Referat II 6 Bauen und Umwelt<br/>
        Straße des 17. Juni 112<br/>
        10587 Berlin<br/>
    </p>
    <h2>Responsible</h2>
    <p>Stephan Rössig<br/>
        E-Mail: referat-2-6(at)bbr.bund.de<br/>
        Telefon: +49 30 / 18401-3417<br/>
        Telefax: +49 30 / 18401-2769
    </p>

  <h2>Technical support / Development</h2>
  <p>Online Now! GmbH<br/>
Agentur für digitale Kommunikation<br/>
Reichsstraße 100<br/>
14052 Berlin<br/>
    <a class="no-xhr" href="mailto:info@online-now.de">info@online-now.de</a><br/>
    <a class="no-xhr" target="_blank" href="https://www.online-now.de">https://www.online-now.de</a>
  </p>


  <h2>Development (2012-2018)</h2>
  <p>BEIBOB Medienfreunde<br/>
    Tobias Lode und Fabian Möller GbR<br/>
    Stefan-George-Weg 44A<br/>
    64285 Darmstadt<br/>
    <a class="no-xhr" href="mailto:sayhello@beibob.de">sayhello@beibob.de</a><br/>
    <a class="no-xhr" target="_blank" href="http://www.beibob.de">http://www.beibob.de</a>
  </p>


    <h2>Responsibility for the content</h2>
    <p>Bundesinstitut für Bau-, Stadt-und Raumforschung<br/>
        Referat II 6 Bauen und Umwelt</p>

    <p>Despite careful control we assume no liability for the content of external links. are solely responsible for the
        content of linked sites responsible.</p>

    <h2>Open Source / Licensing</h2>
    <p>The application eLCA is licensed under the GNU Affero General Public License version 3 (AGPLv3). This guarantees
        every user the freedoms to use, spread and allowed to modify the software. A complete version of the license is
        the source code in the file COPYING. They can also be viewed using the following link.
        <a class="no-xhr" target="_blank" href="http://www.gnu.org/licenses/agpl.html">http://www.gnu.org/licenses/agpl.html</a>
    </p>

    <p>Source code is available here: <a class="no-xhr" target="_blank" href="https://bitbucket.org/beibob/elca">https://bitbucket.org/beibob/elca</a></p>

</div>
