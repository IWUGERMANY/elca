<?php
/**
 * This file is part of the eLCA project
 *
 * eLCA
 * A web based life cycle assessment application
 *
 * Copyright (c) 2016 Tobias Lode <tobias@beibob.de>
 *               BEIBOB Medienfreunde GbR - http://beibob.de/
 *
 * eLCA is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * eLCA is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with eLCA. If not, see <http://www.gnu.org/licenses/>.
 *
 */

declare(strict_types = 1);
namespace Elca\View\helpers;

use Beibob\Blibs\FrontController;
use Beibob\Blibs\HtmlDOMFactory;
use Beibob\HtmlTools\HtmlDataElement;
use Beibob\HtmlTools\HtmlElement;
use Beibob\HtmlTools\Interfaces\Formatter;
use Beibob\HtmlTools\ObjectPropertyConverter;
use DOMDocument;
use Elca\Controller\ProjectData\ProjectAccessCtrl;

class ProjectAccessTokenActions extends HtmlDataElement
{
    /**
     * Builds this element
     *
     * @see HtmlElement::build()
     */
    public function build(DOMDocument $Document)
    {
        $frontController = FrontController::getInstance();
        $factory         = new HtmlDOMFactory($Document);

        $dataObject = $this->getDataObject();
        $tokenId = $dataObject->tokenId;

        $ul = $factory->getUl();
        $li = $ul->appendChild($factory->getLi());

        if ($dataObject->isConfirmed()) {
            $li->appendChild(
                $factory->getA(
                    [
                        'href' => $frontController->getUrlTo(
                            ProjectAccessCtrl::class,
                            'toggleEdit',
                            ['tokenId' => $tokenId]
                        ),
                    ],
                    $dataObject->canEdit ? 'Editieren verbieten' : 'Editieren erlauben'
                )
            );
        }

        $li = $ul->appendChild($factory->getLi());
        $li->appendChild(
            $factory->getA(
                [
                    'href' => $frontController->getUrlTo(ProjectAccessCtrl::class, 'remove', ['tokenId' => $tokenId]),
                ],
                'entfernen'
            )
        );

        /**
         * Set remaining attributes
         */
        $this->buildAndSetAttributes($ul, $dataObject, $this->getName());

        return $ul;
    }
    // End build


}
