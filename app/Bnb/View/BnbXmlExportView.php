 ...
 */
class BnbXmlExportView extends HtmlView
{
    /**
     * Initialize view
     *
    ...
    protected function afterRender()
    {
        ...
        {
            $Container->appendChild($this->getH3(t('eBNB-Export herunterladen')));

            if (is_object($this->get('OriginalProjectVariant')) && $this->get('OriginalProjectVariant')->isInitialized())
            {
                $P = $Container->appendChild($this->getP(t('Eine Kopie von') . ' "'));
                $P->appendChild($this->getI($this->get('OriginalProjectVariant')->getName()));
                $P->appendChild($this->getText('"' . t('%item% wurde erstellt!', null, ['%item%' => ''])));
            }
            ...
            $Form = new HtmlForm('bnbExport', '/bnb/xml-export/');
        ...
        {
            $Form = new HtmlForm('bnbExport', '/bnb/xml-export/create/');
            $Form->addClass('clearfix highlight-changes');
            ...
            $Group->add(new ElcaHtmlFormElementLabel(t('Eine Kopie der gewählten Projektvariante erstellen'), new HtmlCheckbox('copyVariant', null, '', false)));
