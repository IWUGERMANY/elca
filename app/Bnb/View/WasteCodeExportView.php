 ...
 */
class WasteCodeExportView extends HtmlView
{
    private $downloadUrl;
    ...
    {
        $container = $this->appendChild($this->getDiv(['id' => 'content', 'class' => 'bnb-wastecode-export']));

        $data = ElcaCacheDataObjectSet::findProcessConfigMassByProjectVariantId($this->projectVariantId,
            ['mass' => 'DESC']);

        $table = new HtmlTable('wastecode-data');
