...
use IfcViewer\Controller\MainCtrl;

class IfcViewerProjectNavigation implements ElcaNavigationInterface
{
    ...
    {
        $navigation = ElcaNavigation::getInstance('ifc-viewer-project-data');

        $item = $navigation->add(t('IfcViewer'));
        $item->add(t('IfcViewer'), 'ifcViewer', MainCtrl::class, 'default', [
            'target' => '_blank'
        ]);
