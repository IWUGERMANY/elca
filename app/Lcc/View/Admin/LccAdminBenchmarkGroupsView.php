 ...
 */
class LccAdminBenchmarkGroupsView extends HtmlView
{
    /**
    ...
    protected function beforeRender()
    {
         ...
         * Add Container
         */
        $container = $this->appendChild($this->getDiv(['id' => 'tabContent', 'class' => 'elca-admin-benchmark-version tab-lcc-groups active benchmark-groups']));

        $container->appendChild($this->getP(t('Sie können für jede Sonderbedingung eine Gruppe von Bewertungskategorien definieren.')));

        $formId = 'adminBenchmarkGroupsForm';
        $form   = new HtmlForm($formId, '/lcc/admin/benchmarks/saveVersionGroups/');
    ...
    protected function appendGroups(HtmlForm $Form)
    {
        ...
        $row->add(new HtmlTag('h5', t('Sonderbedingung'), ['class' => 'hl-indicators']));
        ...
        foreach (LccBenchmarkGroupSet::findByBenchmarkVersionId($this->benchmarkVersionId, ['id' => 'ASC']) as $benchmarkGroup) {
     ...
     * @param LccBenchmarkGroup $group
     */
    private function appendGroup(HtmlElement $container, LccBenchmarkGroup $group = null)
    {
        $key = null !== $group ? $group->getId() : 'new';

        $container->add(new ElcaHtmlFormElementLabel('', new HtmlTextInput('name['.$key.']')));
        $container->add(new ElcaHtmlFormElementLabel('', $radioGroup = new HtmlRadioGroup('category['.$key.']')));
    ...
    private function appendGroupThresholds(HtmlElement $container, $groupKey)
    {
        ...
        if (is_numeric($groupKey)) {
            ...
            $li = null;
            foreach (LccBenchmarkGroupThresholdSet::findByGroupId($groupKey, ['score' => 'DESC']) as $groupThreshold) {
            ...
            if ($this->get('addNewGroupThreshold', false) && $this->get('groupId') == $groupKey) {
                ...
                $benchmarkVersionId = LccBenchmarkGroup::findById($groupKey)->getBenchmarkVersionId();
                /**
                ...
                $li->add(
                    $Link = new HtmlLink(
                        '+ '.t('Bewertung'),
                        Url::factory('/lcc/admin/benchmarks/addGroupThreshold/', [
     ...
     * @param HtmlElement              $container
     * @param                          $groupKey
     * @param LccBenchmarkGroupThreshold $groupThreshold
     */
    private function appendGroupThreshold(HtmlElement $container, $groupKey, LccBenchmarkGroupThreshold $groupThreshold = null)
    {
        ...
        if ((isset($this->data->score[$key]) && $this->data->score[$key]) ||
            ...
            if (is_numeric($groupKey) && $groupThreshold instanceof LccBenchmarkGroupThreshold) {
                $container->add(
                    $Link = new HtmlLink(
                        t('Löschen'),
                        Url::factory('/lcc/admin/benchmarks/deleteGroupThreshold/', [
                ...
                $container->add($Link = new HtmlLink(t('Abbrechen'), Url::factory('/lcc/admin/benchmarks/editVersionLccGroups/', ['id' => $this->benchmarkVersionId])))
